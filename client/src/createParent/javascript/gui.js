...
function displayParents() {
    ...
    const parentButton = createAddButton('createParent', 'Tilføj Forældre', () => {
        ...
        createParent("test", "23840234", "adflkgs@gmail.com");
        displayParents();
    });
...
function createParentElement(parent, index) {
    ...
    headerDiv.innerHTML = `<h2>${parent.name}</h2>
    ...
    headerDiv.querySelector('.delete').addEventListener('click', () => deleteParentHandler(index));
